...
Case 把一次马上要完成的事项固定成短周期目标容器：目标、边界和验收标准写入 `case.md` 的 Case Contract，再按 Think / Design / Execute / Verify / Close 推进。Step 的语义调整为 Work Package：它不是微步骤，而是围绕设计边界的可执行、可恢复、可验收目标包。飞书 OKR、分身 Agent、执行记录、归档和 Close 归档候选都围绕这条执行线工作。
