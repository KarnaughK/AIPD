...
> Schema 说明（2026-08-10）：正文中的 `_adoc`、L1-L5、Step 等名称按当时历史原样保留。当前运行时等价入口是 `_aipd/`、五类并列 Knowledge 和 Work Package；现行定义以 `_aipd/index.md`、`_aipd/map.md` 与 `_aipd/knowledge/core/workspace-modules.md` 为准。
...
- 新增 AIPD 自迭代能力，形成 c3 agent fork role policy。
...
- 按 C7 思路重构项目结构，将 skill 源码移动到 `aipd-skill/`。
...
- 新建 AIPD 自身 `_adoc/`、AGENTS、OKR、case index 和 c1 Codex custom agents case。
...
- 新增 c2 subagent origin model case。
- README 小幅更新。
...
- 更新 AIPD 自迭代相关 skill。
- 新增 c3 agent fork role policy case 和相关研究 / 设计 step。
- 新增 Vue 架构图指引。
...
- 按 C7 思路重构项目结构，将源码集中到 `aipd-skill/`。
- 清理 legacy 历史材料。
...
- 新增 c9 AIPD Think system design case，用于研究 OpenSpec 风格流程和同类框架。
- 刷新 C7 AIPD 认知层与流程边界。
...
### 2. Knowledge 分类与 Map 是否服务任务检索

AIPD 从 `spec/system/runbook` 发展到 L1-L5，最终把 Intent、Research、Core、Product、Engineering 明确为五类并列 Knowledge。存储分类的目的不是规定读取顺序；Map 才负责让 Agent 按任务场景找到正确上下文。

观察点：

- `_aipd/map.md` 是否能一跳路由到关键文档和代码入口。
- 项目总图、业务线 / 功能线 / shared capability 上下文 Map、局部实现图是否按需要暴露，而不是变成固定三级跳转。
- 五类 Knowledge 的边界是否帮助稳定写回，还是变成“这个东西到底放哪”的分类消耗。
...
### 6. Case Think 是否防止过早进入 Execute

Think 已收敛为 Case 内 phase，用来避免信息不足时过早进入 Design / Execute。后续观察：

- Think 是否有明确状态和出口。
- Think 是否能输出 Create / Kill / Defer / Research / Weave / Continue，而不是默认走 Create。
- Think 是否让设计与执行更准，而不是新增一个每个 Case 都必须膨胀的填写层。
...
- 本日志记录已提交历史；未提交的 Case 执行过程仍留在对应 Case / Work Package 文件中。
