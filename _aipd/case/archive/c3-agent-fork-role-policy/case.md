# Case: c3-agent-fork-role-policy
...
- `_adoc/case/c2-subagent-origin-model/case.md` - 分身 Agent / 克隆体模型的上一轮结论。
...
- [x] `steps/wp-01-research-codex-agent-layering.md` - 调研 Codex fork_context 与 custom agent 角色叠加机制（推荐 Agent：explorer）
- [x] `steps/wp-02-design-dispatch-policy.md` - 设计分身 Agent / 角色 Agent / Main Agent 直接执行的调配策略（推荐 Agent：worker）
- [x] `steps/wp-03-update-docs-if-needed.md` - 根据策略修改 agent guide、case-run 和 Agent Entry（推荐 Agent：worker）
...
- [x] 如需改文档，更新位置和规则清楚，不推翻 c2 的核心认知。
...
- [ ] 如果修改 case-run 或 Agent Entry，是否避免推翻 c2 的克隆体核心认知。
