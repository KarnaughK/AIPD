# Case: c5-human-docs-architecture
...
```text
_adoc/case/c5-human-docs-architecture/
├── case.md
├── steps/
│   └── wp-01-design-human-docs-ia.md
└── doc/
```
...
- [x] `steps/wp-01-design-human-docs-ia.md` - 设计面向人的 `docs/` 渐进式目录、阅读路径和迁移策略（推荐 Agent：worker）
- [x] `steps/wp-02-create-docs-and-promote-readme.md` - 创建正式 docs，升级新版 README，并将旧 README 内容迁移 / 改写到 docs（推荐 Agent：worker）
