...
- **Research**：外部 Agent Skill、产品设计方法、原型工作流、开源生态采用度。
- **Core**：AIPD 的任务执行模型、SOP / AI 程序模型、文件优先 checkpoint。
- **Product**：产品文档与产品原型设计将成为何种用户可调用能力。
- **Engineering**：后续若实现，需要决定 Skill 注入、phase gate、构建与平台适配。
- **局部 README**：当前不涉及页面代码；候选源码只作为 Think 证据。
- **Case / 历史 Work Package**：参考现有经验库边界与 `c18` 的文档产品经验，不读取无关历史 case。

#### 项目认知

- `_aipd/index.md` - 项目状态与认知入口。
- `_aipd/map.md` - 命中实践经验库、SOP、Case Think、Learn / Weave 入口。
- `_aipd/knowledge/core/horizontal-capabilities.md` - Case、SOP、文件 checkpoint 和经验回写关系。
- `_aipd/knowledge/product/index.md` - AIPD 现有产品能力与 Skill / SOP 边界。
...
- `rg "prototype|原型|产品设计|UX|PRD|experience|SOP" aipd-skill/src _aipd docs` - 地图未覆盖时定位现有规则和潜在冲突。
...
- [ ] Agent 是否读取本 Case 上下文索引中的必要 Knowledge / 局部 README。
