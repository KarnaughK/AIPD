...
把已通过本地验收的 Vue3 源码资产和语义经验隔离发布到 AIPD GitHub，记录真实 latest 与包含资产的完整 commit SHA pinned 地址，并让 c13 具备重新 Close 的远端证据。
...
截至 2026-07-15 01:32 CST，上述授权连续三轮未取得；远端没有 c13 分支，默认分支资产 URL 仍为 404。本 work package 停在进入条件前，不执行任何提交、推送、合并或安装。
...
- 只纳入 c13、五条 Vue3 正式经验、`experience-assets/` 和本次必需的长期索引 / 规则。
- 不混入 c12、Agent 路由和用户其他未提交改动。
- 当前分支包含 c12 已有提交与脏改动；发布时从 `origin/main` 创建独立 `codex/c13-vue3-experience-assets` worktree / 分支，不直接在当前工作树整体提交。
- 不提交或推送来源业务项目；其 `AipdModalBox` 迁移继续保留在该项目工作树，由用户另行决定版本控制动作。

## 执行与验证

1. 在隔离 worktree 中重放并复核 c13 文件集合，运行 `node experience-assets/scripts/verify-assets.mjs --full` 和 `git diff --check`。
