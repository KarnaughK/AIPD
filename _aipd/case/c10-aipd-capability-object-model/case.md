# Case: c10-aipd-capability-object-model
...
>
> **收口结论（2026-06-27）**：本 case 不再继续推进为独立的能力层重构事项。后续 AIPD 已收敛到 contract + phase-first case 生命周期：`Case Contract -> Think -> Design -> Execute -> Verify -> Close`。Think 不再作为独立 `_aipd/think/` 系统或独立 `aipd-think` skill 推进，而是作为 Case 内 phase 承接当前目标下的信息同步、调研、比较和抉择。本文保留为原子能力和中间对象模型的参考材料，不作为待执行 case。
>
...
```text
_aipd/case/c10-aipd-capability-object-model/
├── case.md
├── steps/
...
- **项目类型**：AIPD 框架项目。
- **Case 类型**：Core 核心模型 + Product 产品能力 + Engineering Skill 架构边界的底层架构设计 Case。
- **适用流程**：目标型 / 认知型 case。当前先沉淀讨论和设计框架，不直接实现能力层。
- **不适用经验**：不套用 Vue 前端实现流程，不创建代码执行 step，不把外部开源项目直接接入 AIPD。
- **和 c9 的关系**：`c9-aipd-think-system-design` 证明 Think 需要前置讨论和决策对象；本 case 进一步把 Think 中暴露的上下文扫描、深度检索、方案比较、handoff、Close 归档候选 等能力抽象成跨流程可复用原子能力。

## 3. 上下文索引

### 认知域判断

- **Research**：涉及外部开源项目、行业实践、agent workflow、spec-driven development、search / RAG / repo map 工具。
- **Core**：涉及 AIPD 核心成立模型的重构，尤其 Think / 任务澄清决策模型、任务执行模型、项目知识库维护模型、Map-first 上下文检索模型、Agent 协作思考模型、SOP / AI 程序模型。
- **Product**：涉及 AIPD Think、Case Create、Case Run、Weave、SOP 这些产品能力如何调用统一原子能力。
- **Engineering**：后续实现会影响 Skill 目录、模板、Agent 调度、外部 adapter、分身 Agent 调用和可能的模型选择策略。
- **Case / 历史 Step**：需要读取 `c9-aipd-think-system-design` 的调研和设计文档作为前置依据。

### 项目认知

- `_aipd/index.md` - AIPD 项目入口和 Knowledge / Case / SOP 路由。
- `_aipd/map.md` - Think、Case、Weave、SOP、高频能力入口。
- `_aipd/knowledge/core/index.md` - AIPD 核心成立模型。
- `_aipd/knowledge/core/horizontal-capabilities.md` - Think、Case、SOP、Weave、map 等横向能力关系。
- `_aipd/knowledge/core/workspace-modules.md` - Knowledge、Map、SOP、Case、OKR、Inbox、Agent Entry 与真实代码的 Workspace 模块。
- `_aipd/knowledge/product/index.md` - AIPD 产品能力总览。
- `_aipd/knowledge/product/map.md` - 产品功能线地图。
- `_aipd/knowledge/engineering/index.md` - Codex / Agent / skill 架构和工程约束。

### 相关 case / doc

- `_aipd/case/archive/c9-aipd-think-system-design/case.md` - Think 调研设计 case。
- `_aipd/case/archive/c9-aipd-think-system-design/doc/openspec-think-analysis.md` - OpenSpec 机制分析。
- `_aipd/case/archive/c9-aipd-think-system-design/doc/peer-framework-survey.md` - Spec Kit、BMAD、Open SWE、Kiro、Cline 等同类资料调研。
- `_aipd/case/archive/c9-aipd-think-system-design/doc/aipd-boundary-comparison.md` - Inbox / Think / Case / SOP / Weave 边界矩阵。
- `_aipd/case/archive/c9-aipd-think-system-design/doc/think-object-state-design.md` - Think 对象和状态流转设计。
- `_aipd/case/archive/c9-aipd-think-system-design/doc/handoff-and-skill-boundary.md` - Think -> Case Create handoff 和 skill 边界设计。
- `_aipd/case/c10-aipd-capability-object-model/doc/discussion-seed.md` - 本 case 的原始讨论种子。

### 兜底搜索

- `rg "Think|Case Create|Case Run|Weave|SOP|Context Pack|Research Pack|Decision Record|Handoff|Close 归档候选" _aipd aipd-skill/src docs`
- `rg "OpenSpec|Spec Kit|BMAD|Kiro|Cline|Open SWE|OpenHands|requirements analysis|Plan Act|artifact DAG" _aipd docs aipd-skill/src`
...
`research.deep` 不只 Think 需要，Research、技术选型、外部框架吸收也需要。
...
| 对象 | 可能用途 |
|---|---|
| Context Pack | 上下文扫描结果，按相关性列出五类 Knowledge / Case / SOP / README / 真实代码 / 外部入口 |
...
| Close 归档候选 | 可能进入五类 Knowledge / README / Map 的稳定认知 |
...
- **不再单独推进**：不再从本 case 派生独立 Capability Registry、独立 `_aipd/think/` 或独立 `aipd-think` skill。
...
- 不接入任何外部开源项目。
- 不推翻 c9 Think 设计；本 case 是在 c9 之上进一步抽象。
- 不把所有开源项目当成 AIPD 的依赖。
...
- **能力注册表**：是否需要 `_aipd/capabilities/`、`aipd-skill/src/core/capabilities/` 或 SOP 形式承载能力说明。
...
- [x] 已明确本 case 与 c9 Think 体系设计的关系。
- [x] 已创建后续设计 step，但未开始执行重构。

## 9. Weave 反向编织候选

- `_aipd/knowledge/core/index.md` - 若本方向确认，可新增或调整 AIPD 核心成立模型，补充 Capability Object Model / 原子能力层。
- `_aipd/knowledge/core/horizontal-capabilities.md` - 可补充原子能力层如何支撑 Think / Case / Weave / SOP。
- `_aipd/knowledge/product/index.md` - 如果原子能力层成为用户可调用 / Agent 可组合能力，可新增产品能力边界。
- `_aipd/map.md`、`_aipd/knowledge/core/map.md` - 后续补“原子能力 / Capability Object Model / Context Pack / Research Pack”等检索入口。
