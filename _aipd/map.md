...
| `$aipd-leader` / Project Leader / 项目负责人 / 监督者 / 多开线 / 阶段汇报 / AI 主导项目 / Mission / 主导模式 / 跨 Case 推进 / Codex task / cursor-agent / 思路变化 / 回跳校准 / 等 Codex 线程 / 等待 Case task | AIPD Leader（显式可选） | 显式调用时进入 Leader Mission 与流程状态；其他说法只用于解释或建议，不自动启动 | `_aipd/leader/index.md`、`aipd-skill/src/skills/aipd-leader/SKILL.md` | `aipd-skill/src/core/leader/`、`aipd-skill/src/platforms/codex/core/leader/runtime.md`、`aipd-skill/src/platforms/cursor/core/leader/runtime.md`、Leader 自建文字文件 | `rg -e "aipd-leader" -e "Project Leader" -e "cursor-agent" -e "chatId" -e "阶段汇报" -e "Mission" -e "Codex task" -e "wait_threads" -e "当前工作区索引" -e "反向归属" -e "跨 Case" _aipd/leader _aipd aipd-skill/src docs` |
| 目标模式 / goal 模式 / 绑目标 / 自己定义目标 / `/goal` / CreateGoal / Leader 等 Case 时开 goal | 当前 Agent 平台的目标模式 | 先看本对话身份，再进对应 Skill；不凭项目里已有 Mission 自称 Leader。Codex Leader 等待独立 Case task 时必须给自己开或确认绑 Mission 的 goal | `AGENTS.md` 中「目标模式 / goal 模式」；未显式 Leader 时 `_aipd/case/index.md`，已是 Leader 时 `_aipd/leader/index.md` | 执行层：`aipd-skill/src/skills/aipd-case/SKILL.md`、`aipd-skill/src/core/case/goal-mode.md`；Leader：`aipd-skill/src/skills/aipd-leader/SKILL.md`、`aipd-skill/src/core/leader/guide.md`、`aipd-skill/src/platforms/codex/core/leader/runtime.md` | `rg -e "目标模式" -e "goal 模式" -e "/goal" -e "create_goal" -e "wait_threads" AGENTS.md aipd-skill/src _aipd` |
| inbox / 收件箱 / 先记一下 / 先存一下 / 回头再整理 / 扔一边 / 回头有空 | Inbox 临时收件箱 | capture | `_aipd/inbox.md` | `aipd-skill/src/skills/aipd-inbox/SKILL.md`、`aipd-skill/src/core/workspace/templates/inbox.md` | `rg "inbox|收件箱|先记一下|先存一下|回头再整理|扔一边" _aipd aipd-skill/src` |
...
| case / Case Contract / 目标边界 / case design / case think / work package / 创建 case / 执行 case / 验收 / 归档 / 推进到结束 / 回跳 | AIPD Case 统一入口 | case 流程 + Core/Product/Engineering 能力文档 | `_aipd/case/index.md`、`_aipd/knowledge/core/horizontal-capabilities.md`、`_aipd/knowledge/product/map.md` | `aipd-skill/src/skills/aipd-case/SKILL.md`、`aipd-skill/src/core/case/overview.md`、`aipd-skill/src/core/case/goal-mode.md`、`aipd-skill/src/core/case/phases/`、`aipd-skill/src/core/case/templates/case.md`、`aipd-skill/src/core/case/templates/work-package.md` | `rg "aipd-case|Case Contract|Case Phase|Work Package|复杂度爆点|case design|case think|回跳|goal-mode|goal 模式" aipd-skill/src _aipd/case docs` |
...
| AIPD Leader / Leader 工作记忆 | `$aipd-leader`、`_aipd/leader/` | `_aipd/leader/index.md`、`aipd-skill/src/skills/aipd-leader/SKILL.md` | 显式 Leader 模式、受托项目监督、一个 active Mission、Codex 上一个 Case 一个独立 task、等待独立 Case task 时更新进度并给 Leader 自己开绑 Mission 的 goal、Cursor 上因 Agent 不够强才用一条 `cursor-agent` 对话、阶段汇报 | 不是第六类 Knowledge，也不自动启动；Leader 负责项目级判断和汇报，Case 负责细节执行与 phase 回跳；goal 模式按本对话身份绑 Mission 或 Case，不替代二者；Codex Leader 等 Case task 时必须给自己开 goal，不要替执行层开；不要把 Cursor 桌面端组合套到 Codex；Cursor 不找 DSH；工作记忆只保存没有更权威归属、但会影响当前判断和恢复的信息 |
...
| 目标模式 / goal 模式 / 绑目标 / `/goal` / Leader 等 Case 时开 goal | 当前 Agent 平台的目标模式 | `_aipd/knowledge/product/map.md` | `AGENTS.md`；按本对话身份进入 `aipd-leader` 或 `aipd-case` | 本对话的宿主 goal；Leader 绑 Mission，执行层绑当前 Case。Codex Leader 等待独立 Case task 时必须给自己开或确认绑 Mission 的 goal | `_aipd/knowledge/core/horizontal-capabilities.md` | `aipd-skill/src/core/leader/guide.md`、`aipd-skill/src/platforms/codex/core/leader/runtime.md`、`aipd-skill/src/core/case/goal-mode.md` |
