...
| AIPD Leader / Leader 工作记忆 | `$aipd-leader`、`_aipd/leader/` | `_aipd/leader/index.md`、`aipd-skill/src/skills/aipd-leader/SKILL.md` | 显式 Leader 模式、受托项目监督、一个 active Mission、Codex 上一个 Case 一个 task、Cursor 上一个 Case 一条 `cursor-agent` 对话、阶段汇报 | 不是第六类 Knowledge，也不自动启动；Leader 负责项目级判断和汇报，Case 负责细节执行与 phase 回跳；goal 模式按本对话身份绑 Mission 或 Case，不替代二者；Cursor 不找 DSH；工作记忆只保存没有更权威归属、但会影响当前判断和恢复的信息 |
