...
| `$aipd-leader` / Project Leader / 项目负责人 / 监督者 / 多开线 / 阶段汇报 / AI 主导项目 / Mission / 主导模式 / 跨 Case 推进 / Codex task / cursor-agent / 思路变化 / 回跳校准 | AIPD Leader（显式可选） | 显式调用时进入 Leader Mission 与流程状态；其他说法只用于解释或建议，不自动启动 | `_aipd/leader/index.md`、`aipd-skill/src/skills/aipd-leader/SKILL.md` | `aipd-skill/src/core/leader/`、`aipd-skill/src/platforms/codex/core/leader/runtime.md`、`aipd-skill/src/platforms/cursor/core/leader/runtime.md`、Leader 自建文字文件 | `rg -e "aipd-leader" -e "Project Leader" -e "cursor-agent" -e "chatId" -e "阶段汇报" -e "Mission" -e "Codex task" -e "当前工作区索引" -e "反向归属" -e "跨 Case" _aipd/leader _aipd aipd-skill/src docs` |
| 目标模式 / goal 模式 / 绑目标 / 自己定义目标 / `/goal` / CreateGoal | 当前 Agent 平台的目标模式 | 先看本对话身份，再进对应 Skill；不凭项目里已有 Mission 自称 Leader | `AGENTS.md` 中「目标模式 / goal 模式」；未显式 Leader 时 `_aipd/case/index.md`，已是 Leader 时 `_aipd/leader/index.md` | 执行层：`aipd-skill/src/skills/aipd-case/SKILL.md`、`aipd-skill/src/core/case/goal-mode.md`；Leader：`aipd-skill/src/skills/aipd-leader/SKILL.md`、`aipd-skill/src/core/leader/guide.md` | `rg -e "目标模式" -e "goal 模式" -e "/goal" AGENTS.md aipd-skill/src _aipd` |
...
| AIPD Leader / Leader 工作记忆 | `$aipd-leader`、`_aipd/leader/` | `_aipd/leader/index.md`、`aipd-skill/src/skills/aipd-leader/SKILL.md` | 显式 Leader 模式、受托项目监督、一个 active Mission、Codex 上一个 Case 一个独立 task、Cursor 上因 Agent 不够强才用一条 `cursor-agent` 对话、阶段汇报 | 不是第六类 Knowledge，也不自动启动；Leader 负责项目级判断和汇报，Case 负责细节执行与 phase 回跳；goal 模式按本对话身份绑 Mission 或 Case，不替代二者；不要把 Cursor 桌面端组合套到 Codex；Cursor 不找 DSH；工作记忆只保存没有更权威归属、但会影响当前判断和恢复的信息 |
...
| 目标模式 / goal 模式 / 绑目标 / `/goal` | 当前 Agent 平台的目标模式 | `_aipd/knowledge/product/map.md` | `AGENTS.md`；按本对话身份进入 `aipd-leader` 或 `aipd-case` | 本对话的宿主 goal；Leader 绑 Mission，执行层绑当前 Case | `_aipd/knowledge/core/horizontal-capabilities.md` | `aipd-skill/src/core/leader/guide.md`、`aipd-skill/src/core/case/goal-mode.md` |
