...
| AIPD Leader / Leader 工作记忆 | `$aipd-leader`、`_aipd/leader/` | `_aipd/leader/index.md`、`aipd-skill/src/skills/aipd-leader/SKILL.md` | 显式 Leader 模式、受托项目监督、一个 active Mission、一个 Case 一个 Codex task、阶段汇报 | 不是第六类 Knowledge，也不自动启动；Leader 负责项目级判断和汇报，Case 负责细节执行；工作记忆只保存没有更权威归属、但会影响当前判断和恢复的信息 |
