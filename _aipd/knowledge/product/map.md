...
| `$aipd-leader` / Leader 模式 / 项目负责人 / 监督者 / 多开线 / 阶段汇报 / AI 主导项目 / Mission / 跨 Case 调度 / Codex task | AIPD Leader | 已存在 | `aipd-skill/src/skills/aipd-leader/SKILL.md` | `_aipd/leader/`、Mission、用户关注 / 汇报约定、Case / Codex task 绑定 | Leader 项目主导编排模型、任务执行模型 | `aipd-skill/src/core/leader/`、`aipd-skill/src/platforms/codex/core/leader/runtime.md` |
...
| AIPD Leader | 受托监督一个 active Mission，探索方向、调度一个 Case 一个 Codex task、阶段汇报并做总验收 | 不自动启动，不替代 Case 的细节执行，不并发多个 Leader / Mission，不扩大外部副作用权限 |
