...
| `$aipd-leader` / Leader 模式 / 项目负责人 / 监督者 / 多开线 / 阶段汇报 / AI 主导项目 / Mission / 跨 Case 调度 / Codex task / cursor-agent | AIPD Leader | 已存在 | `aipd-skill/src/skills/aipd-leader/SKILL.md` | `_aipd/leader/`、Mission、用户关注 / 汇报约定、Case / Codex 独立 task；Cursor 上才是 `chatId` 绑定 | Leader 项目主导编排模型、任务执行模型 | `aipd-skill/src/core/leader/`、`aipd-skill/src/platforms/codex/core/leader/runtime.md`、`aipd-skill/src/platforms/cursor/core/leader/runtime.md` |
| 目标模式 / goal 模式 / 绑目标 / `/goal` / CreateGoal | 当前 Agent 平台的目标模式 | 已存在 | `AGENTS.md`；按本对话身份进入 `aipd-leader` 或 `aipd-case` | 本对话的宿主 goal；Leader 绑 Mission，执行层绑当前 Case | Leader 项目主导编排模型、任务执行模型 | `aipd-skill/src/core/leader/guide.md`、`aipd-skill/src/core/case/goal-mode.md` |
