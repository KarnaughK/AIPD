...
| AIPD Leader | 受托监督一个 active Mission，探索方向、按宿主调度 Case 执行层（Codex task 或 Cursor `cursor-agent`）、阶段汇报并做总验收 | 不自动启动，不替代 Case 的细节执行与回跳，不并发多个 Leader / Mission，不为推完一个 Case 再开平级 Case，不找 DSH，不扩大外部副作用权限 |
