...
| `$aipd-leader` / Mission / 项目负责人 / 多 Case task / cursor-agent | Leader 项目主导编排模型 | 用户如何显式委托一个 AI Leader 负责一个 active Mission 的方向探索、Case 执行层协调和总验收 | `_aipd/knowledge/core/index.md`、`_aipd/knowledge/core/horizontal-capabilities.md`、`aipd-skill/src/core/leader/guide.md` | AIPD Leader | 不因任务复杂、提到 Leader 或存在多个 Case 自动启动；goal 模式按本对话身份绑 Mission 或 Case，不替代二者；phase 回跳在 Case 执行层，Leader 只打回同一执行层；Cursor 上执行层是 `cursor-agent`，不是 DSH 或对话内子 Agent |
