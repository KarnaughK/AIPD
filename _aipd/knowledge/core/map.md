...
| Main Agent / 分身 Agent / 角色 Agent / fork_context | Agent 协作思考模型 | 多个 Agent 如何分工探索、保护主线判断，并回流压缩结论 | `_aipd/knowledge/core/index.md`、`_aipd/knowledge/engineering/index.md`、`aipd-skill/src/platforms/codex/core/agent-guide.md` | Case Execute、Agent 调度 | 分身 Agent 不是低上下文执行工人 |
...
| AIPD Workspace / 工作区 | Workspace 模块 | 把 Knowledge、Map、SOP、Case、OKR、Inbox 和 Agent Entry 放在统一可发现入口下 | `_aipd/knowledge/core/workspace-modules.md` | AIPD 初始化、aipd-case | 不是代码拓扑中的纵向业务上下文；真实代码不复制进 Workspace |
...
- `rg "上下文解耦|黑箱上移|扁平化检索|Weave|分身 Agent|Core" _aipd src`
