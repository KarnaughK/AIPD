...
| `$aipd-leader` / Mission / 项目负责人 / 多 Case task / cursor-agent | Leader 项目主导编排模型 | 用户如何显式委托一个 AI Leader 负责一个 active Mission 的方向探索、Case 执行层协调和总验收 | `_aipd/knowledge/core/index.md`、`_aipd/knowledge/core/horizontal-capabilities.md`、`aipd-skill/src/core/leader/guide.md` | AIPD Leader | 不因任务复杂、提到 Leader 或存在多个 Case 自动启动；goal 模式按本对话身份绑 Mission 或 Case，不替代二者；phase 回跳在 Case 执行层，Leader 只打回同一执行层；Codex 上直接开独立 task；Cursor 上执行层才是 `cursor-agent`，不是 DSH 或对话内子 Agent |
| 目标模式 / goal 模式 / 绑目标 / `/goal` / CreateGoal | 当前 Agent 平台的目标模式 | 当前这条 Agent 对话的宿主长效续跑能力；先看本对话是否已显式 Leader，再决定绑 Mission 还是 Case | `_aipd/knowledge/core/horizontal-capabilities.md`、`aipd-skill/src/core/agent-entry/template.md`、`aipd-skill/src/core/leader/guide.md`、`aipd-skill/src/core/case/goal-mode.md` | AIPD Leader、AIPD Case | 不是只能绑 Case 的覆盖层；也不凭项目里已有 Mission 自称 Leader |
