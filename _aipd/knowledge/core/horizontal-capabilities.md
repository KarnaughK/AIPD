...
Case 执行层内部仍可以按项目规则使用 Child Agent 或工具。执行层不创建新的同级 Case task，也不承担跨 Case 方向判断。同一 Case 的 phase 回跳留在该执行层；Leader 没有第二套 phase 机。平台目标是宿主完成合同，不是第三套运行时：Leader 上写 Mission，Case 上最多绑这一个 Case。
