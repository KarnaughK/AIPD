# Case: c7-repo-structure-reclassification
...
```text
_adoc/case/archive/c7-repo-structure-reclassification/
├── case.md
├── steps/
...
- **为 c8 Desktop 铺路**：后续 AIPD Desktop case 应基于重构后的 `aipd-desktop/` 位置继续迭代。
...
- **Case 依赖**：后续 `c8-aipd-desktop-zero` 应在本 case 完成后更新路径假设。
...
├── aipd-desktop/   # 新桌面端：Tauri + Vue + Rust，后续 c8 创建
├── docs/           # 面向人的文档
├── AGENTS.md
├── README.md
└── .gitignore
...
- 已创建 Git tag `c7-before-legacy-cleanup` 指向清理 legacy 前的稳定提交，便于回看 v1 / v2-todo 旧材料。
- `docs/legacy/` 已从工作树删除；旧 v1 / v2-todo 材料通过 Git tag `c7-before-legacy-cleanup` 追溯，`_adoc/inbox.md` 保留待判断索引。
...
- 本 case 完成后，更新 `c8-aipd-desktop-zero` 中所有 Desktop 路径和前置依赖。
...
- C8 Desktop case 继续基于当前 `aipd-skill/` 与 `aipd*` 命名推进。
...
- [x] legacy 旧材料已通过 `c7-before-legacy-cleanup` 保留快照，当前工作树不再保留 `docs/legacy/`。
- [x] README、docs、_adoc map 和相关 case 路径已同步。
- [x] 未创建 Desktop 项目；Desktop 创建留给后续 `c8`。
...
- `c8-aipd-desktop-zero` - Desktop case 必须在本 case 完成后基于新路径迭代。
