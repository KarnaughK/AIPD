# Case: c0.1-role-codex-custom-agents
...
- [x] `steps/c0.1.1-unify-codex-context.md` - 统一 Codex 子 Agent 上下文策略（推荐 Agent：worker）
- [x] `steps/c0.1.2-agent-build-install.md` - 让 Codex build/install 处理 custom agents（推荐 Agent：worker）
- [x] `steps/c0.1.3-case-run-agent-selection.md` - 让 step/case-run 支持推荐 Agent（推荐 Agent：worker）
- [x] `steps/c0.1.4-self-adoc-entry.md` - 补齐 AIPD2 自身 `_adoc` 和 `AGENTS.md`（推荐 Agent：worker）
- [x] `steps/c0.1.5-test-vue-agent-dispatch.md` - 创建带 `aipd_vue_architect` 的测试 step（推荐 Agent：aipd_vue_architect）
