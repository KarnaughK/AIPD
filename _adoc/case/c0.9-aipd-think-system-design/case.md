...
> **本次事项目标**：调研并设计 AIPD Think 体系，把“模糊想法 -> 讨论 / 调研 / 方案比较 -> 决策出口 -> Case Create”的前置层设计成一等能力。
...
- **吸收同类项目经验**：重点调研 OpenSpec 的 `/opsx:explore`、proposal / specs / design / tasks / verify / archive 机制，并对比 Spec Kit、BMAD、Open SWE 等同类项目。
...
- **适用流程**：通用 case-create + 调研型 steps。每个 step 输出设计或调研文档，不修改业务代码。
...
- Web 搜索：`OpenSpec explore propose tasks verify`, `spec driven development AI coding assistant`, `BMAD method PRD AI agent`, `Spec Kit clarify plan tasks`
...
- 搜罗并对比同类项目的前置讨论、spec、research、decision、task shaping 机制。
...
- 不把 OpenSpec / Spec Kit / BMAD 原样照搬成 AIPD 流程。
- 不把调研过程直接写入长期 L2/L3/L4；稳定结论等 case 完成后再 weave。

## 5. Step 列表

- [ ] `steps/s1-openspec-think-analysis.md` - 深入分析 OpenSpec 的 explore / artifact DAG / verify / archive 机制。
- [ ] `steps/s2-peer-framework-survey.md` - 调研 Spec Kit、BMAD、Open SWE 等同类项目的前置讨论与 spec 机制。
- [ ] `steps/s3-aipd-boundary-comparison.md` - 对比 AIPD Inbox / Think / Case / SOP / Weave 边界，形成边界矩阵。
- [ ] `steps/s4-think-object-state-design.md` - 设计 Think 对象结构、状态流转和文件化恢复模型。
- [ ] `steps/s5-handoff-and-skill-boundary.md` - 设计 Think -> Case Create 交接格式和 `aipd-think` skill 边界。
...
- [ ] OpenSpec 的 Think 类机制已形成调研报告，并明确哪些可借鉴、哪些不适合照搬。
- [ ] 至少 3 个同类项目 / 框架已对比，覆盖前置讨论、spec、research、decision 或 task shaping 机制。
- [ ] AIPD Inbox / Think / Case / SOP / Weave 边界已形成矩阵，能回答“什么信息放哪里”。
- [ ] Think 对象结构和状态流转已形成设计文档。
- [ ] Think -> Case Create 交接格式和 `aipd-think` skill 边界已形成设计文档。
- [ ] 输出清晰后续建议：是否进入实现 case、需要 weave 到哪些 ADOC、哪些问题暂缓。
