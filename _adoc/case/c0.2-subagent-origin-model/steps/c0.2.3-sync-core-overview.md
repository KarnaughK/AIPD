# Step: c0.2.3 - 同步核心 overview / agent-guide / L5 认知并检查冲突
...
- [x] 更新 `_adoc/L5-dev/index.md` 中 Codex 子 Agent 当前策略。
...
**主要改动**：
- 更新 `src/core/overview.md`，把子 Agent 从独立执行者改为同源分身。
- 更新 `src/core/agent-guide.md`，把平台无关原则改为同源分身、AIPD 通用能力、step 校准和结果回流。
- 更新 `_adoc/L5-dev/index.md` 与 `_adoc/okr/index.md`，同步当前有效策略。
- 检查 `fork_context` 残留：历史 c0.1 记录保留为历史事实，当前有效源码和认知已改为 `fork_context: true`。
