...
本文件是 AIPD2 仓库自身的 `_adoc/` 入口。Agent 进入本项目后，先读这里；再读取 `_adoc/map.md` 做项目记忆检索，最后按任务层级读取对应文档。
...
| L2 Research | `_adoc/L2-research/` | 用户、场景、需求、痛点、竞品、行业/玩法范式和调研结论 | 暂缺 |
| L3 Core | `_adoc/L3-core/` | 核心认知、核心对象、核心关系、核心流程和项目成立模型 | 初始 |
...
- 如果 `map.md` 缺失或未覆盖本次任务，按相关核心词、skill 名、agent 名、平台名、case 名、README 使用 `rg` 兜底搜索，并把新发现的稳定入口回写到合适的 map 或 index。
