...
| AIPD Think | 把模糊想法、陌生领域和“要不要做”的问题状态化，通过讨论、调研、方案比较形成决策出口 | 规划中的 `aipd-think` / Think 对象 | Think / 任务澄清决策模型、Map-first 上下文检索模型 | 规划中 |
| Case Create | 把一次事项写成有目标、边界、上下文索引和 steps 的 case | `aipd-case-create`、case 模板 | 任务执行模型、Map-first 上下文检索模型 | 已存在 |
| Case Run | 按 case / step 恢复状态，推进执行、派发分身 Agent 并验收结果 | `aipd-case-run`、step 文件 | 任务执行模型、Agent 协作思考模型 | 已存在 |
| Case Archive | 在用户验收后整理结果、Weave Candidate、索引和分支合并 | `aipd-case-archive` | 任务执行模型、项目知识库维护模型 | 已存在 |
...
| AIPD Desktop | 用桌面界面组织 AIPD 文件、Case / Step、聊天和预览 | `aipd-desktop/` | Map-first 上下文检索模型、任务执行模型、SOP / AI 程序模型 | 规划中 |
...
AIPD Think 是规划中的前置讨论能力。它不默认创建 case，而是把一个模糊想法变成可恢复、可调研、可决策的 Think 对象。

AIPD Think 的用户价值是提高人和 AI 的沟通带宽：用户不必一次把需求讲清，也不必把深度调研塞在聊天上下文里；AIPD 可以围绕一个 Think 逐步沉淀讨论主线、调研资料、方案比较、风险判断和决策出口。

AIPD Think 的出口包括 Create、Kill、Defer、Research、Weave 和 Continue。只有当出口是 Create 时，才进入 Case Create；Case Create 再负责执行架构、上下文索引、step 拆分和验收设计。

### Case Create / Run / Archive

Case Create 把事项从聊天中固定到文件事实源。Case Run 按 case / step 推进任务执行。Case Archive 在验收后整理结果、经验、Weave Candidate 和归档状态。

这三者共同构成 AIPD 的任务执行产品线。它们不应退化成普通 todo，也不应变成限制 Agent 自由判断的按钮式流程工厂。

### Weave 反向编织

Weave 是项目知识库维护模型的更新能力。它判断讨论、开发、step 结果、错误日志和外部资料中哪些信息值得沉淀，并决定写入 L3 / L4 / L5 / 局部 README / map 的哪个位置。

其中 L3 / L4 / L5 / 局部 README / map 只接收稳定知识；case / step 只保留一次性过程、验收记录和临时决策。普通找代码或查功能时，不应因为 L4 提到某个能力曾由 case 推进，就去读取历史 case。
...
Mermaid / MMD 是高带宽交流能力，用于表达架构关系、Case / Step 拆解、状态流、模块边界和桌面端结构。
