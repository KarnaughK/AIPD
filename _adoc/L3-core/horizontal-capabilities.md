...
AIPD 的任务连续性以文件为核心，不以聊天为核心。聊天上下文只是运行缓存；项目规则、当前状态、设计判断、下一步游标和执行结果应能从 `_adoc/`、case、phase、work package、局部 README 和 map 中恢复。
...
| Weave 反向编织 | case 完成后把已实现、已验收的稳定判断、规则、入口和外部资料写回哪里 | L3、L4、L5、局部 README、map；未完成候选先留在 Close 归档候选 |
...
Weave 负责把已完成事项、已实现功能、已验收规则、case 归档、代码 diff、错误日志和外部资料中产生的新稳定信息，编织回稳定位置。稳定位置指 L3 / L4 / L5、局部 README、总 map 或细节 map；case / work package 只保留一次性过程、验收记录、临时决策、未实现设计和未来计划。
...
Weave 不要求其他执行型 skill 内置完整文档更新逻辑。`aipd-case`、兼容归档入口或普通开发流程只需要把候选放入 Close 归档候选；case 完成后，`aipd-weave` 再统一判断知识归属、索引更新和旧知识冲突。
