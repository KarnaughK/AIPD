...
**_adoc/ 的本质：服务于 AI 的项目知识库**
- L1-L5：按抽象到具象组织，只存整理过的文档
- 不存原始数据、未整理的代码、一次性脚本
- OKR/Plan：人机交互的执行模块，同时服务于 AI
...
- 产出: `_adoc/L1-intent/intent.md`（唯一，一个项目只有一个方向）
...
- 产出: `_adoc/L2-research/` 各需求的调研文档
- 组织方式：每个需求/痛点 = 一个文件夹，包含需求描述 + 拆解 + 分析结果
- 代码与文档分离：调研代码（爬虫、脚本）放项目外独立目录，调研结论归档到 L2
- 这一层重在发散和调研，不急于收敛
- ⚠️ 当前状态: 实践探索中，详细指引待补充

### 第三层：核心层（Core）

从场景需求中提炼最不变的抽象——核心对象、数据模型、核心交互关系。

- 产出: `_adoc/L3-core/` 核心数据模型定义、对象关系
- 核心内容：
  - 核心数据模型
  - 对象关系
  - 交互理念（或核心交互）
  - 设计风格理念（可选，也可放各项目）
  - 可能包含：ER 图、核心实体定义、状态机等
...
- 产出: `_adoc/L4-product/` 各模块的完整定义（Spec + SOP）
