...
/aipd-weave work package 做完了，看看要不要回写
/aipd-weave 这次踩坑沉淀一下
...
2. **Work Package 完成触发**：work package 执行结束后，判断是否出现新规则、新路径、新边界、新坑点。
...
优先使用当前对话、用户刚刚给出的内容、已知执行结果和项目 map，自动总结这次可能要沉淀的重点。不要把“当前讨论 / work package / case / 外部资料 / diff / 错误日志”这种来源分类当成问题抛给用户。
...
|---|---|
| 当前讨论 | 直接基于当前对话提炼候选 |
| work package 结果 | 读取对应 work package 文件、case.md 和执行摘要 |
| case 归档 | 读取 case.md、已完成 work packages、归档前 diff 摘要 |
| 代码 diff | 用 `git diff --stat` 和相关文件 diff 判断是否有文档价值 |
| 外部资料 | 读取用户粘贴内容或指定文件 / URL 摘要 |
| 错误日志 | 只提取可复用原因、定位路径、规避规则 |

如果用户明确给出 work package、step、case、文件、URL 或 diff，就读取对应内容。如果用户没有给出具体来源，就默认从当前对话提炼。
...
- 位置：case、work package、文件路径、transcript 或用户粘贴内容
...
1. **是否稳定**：它必须已经成为当前项目的真实行为或规则，相关实现已经落地，并且有与风险相称的验证 / 验收证据。普通开发也需要测试、构建、人工验收或其他可追溯证据；“已经写了代码”“看起来可复用”或用户只确认回写方案，任何一项都不能单独替代稳定性门槛。不满足时留在 case / work package / Close 候选。
...
```md
【AIPD Weave 回写方案】
来源：当前对话 / work package / case / diff / 外部资料
判定：稳定知识 / 一次性过程 / 两者都有
...
```text
讨论 / work package / case / diff / 外部资料
-> aipd-weave
-> 回写 L3 / L4 / L5 / README / map
-> 一次性过程、验收记录和临时决策留在 case / work package
-> 必要时提示旧知识冲突
-> 下次 Agent 通过正向索引读取到新上下文
```

AIPD 已经有 L1-L6、OKR、Case、Work Package、Agent Entry 等项目级纵向模块。`weave` 的核心价值是把稳定新信息编织进 L1-L5、局部 README 和 map，让项目知识库持续积累；OKR、Case、Work Package 只保留流程状态、一次性过程和恢复记录。
