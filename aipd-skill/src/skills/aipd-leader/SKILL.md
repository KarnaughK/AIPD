...
- 本次显式调用同时授权 Leader：在当前 Mission 内，为已经澄清并确认要推进的 Case 调度同级 Case 执行层。Codex 上这是一个独立 Codex 任务；Cursor 上默认是已登录的 `cursor-agent` 无头进程。它不扩大远端写入、发布、删除、付费或其他外部副作用权限。
...
7. 用户在本 Leader 对话说「目标模式 / goal 模式 / 绑目标 / 自己定义目标」时，为**当前 Mission** 创建当前 Agent 平台的目标模式（Cursor `CreateGoal` / `/goal`，Codex Goal），objective 写 Mission 完成判据。不要绑某一个 Case，不要加载 `case/goal-mode.md`，也不要把这话理解成「Leader 禁止 goal 模式」或「goal 模式只能绑 Case」。Codex 上另有一条强制续跑：本 Leader 已经开出独立 Codex Case task、并且还在等它干活时，必须先更新 `_aipd/leader/` 进度、再确保本对话已有绑 Mission 的 goal 模式，然后才 `wait_threads`；这时不要等用户再说一次「开目标模式」。其他情况仍不要凭任务长、有 Case、有 Mission 或怕压缩而自动创建。
...
10. Leader 可在当前 Mission 内继续下一个已确认 Case，不等待用户逐 Case 验收；遇到方向变化、新权限、破坏性动作、不可逆分歧或跨 Case 冲突时立即暂停并澄清，不等固定汇报节点。
11. 所有 Case 通过后做 Mission 级总验收，向用户说明达成结果、剩余风险、方向变化和下一候选 Mission。
...
- Cursor 上的 Leader 记录实际宿主和 `cursor-agent` 登录状态；不适用 Codex 模型表，也不要假装 Fast 已开启。
