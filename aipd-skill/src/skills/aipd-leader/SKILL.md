...
`aipd-leader` 在默认 Case 执行层之上增加一个项目主导层。Leader 负责方向澄清、探索、Mission、Case 拆分、同级任务调度和总验收；每个 Case 仍由独立任务通过 `aipd-case` 完成。
...
7. 输出一张简短启动卡：当前项目、Leader 状态、当前 Mission、活动 Case、待澄清点、运行配置状态和下一动作。

## Mission 推进

一次只推进一个 active Mission；可以记录排队项，但不要并发两个 Mission。

1. 把用户想达成的结果、项目事实和当前未知项整理成 Mission 草案。
...
4. 在 `_aipd/leader/` 持久化当前 Mission、方向变更依据、Case 队列 / 依赖、任务绑定和下一恢复点。只要求这些信息可恢复，不强制固定文件名；新文件必须回链到 `index.md`。
...
8. Leader 可在当前 Mission 内继续下一个已确认 Case，不等待用户逐 Case 验收；遇到方向变化、新权限、破坏性动作或不可逆分歧时暂停并澄清。
9. 所有 Case 通过后做 Mission 级总验收，向用户说明达成结果、剩余风险、方向变化和下一候选 Mission。
...
退出前至少写回：当前 Mission 状态、未完成 Case 与对应任务、最近一次方向变化、待用户决定事项、下一恢复位置。已经有 Knowledge、Case、OKR、SOP、Map、README 或代码事实源的信息只保留链接和当前影响，不复制正文。
