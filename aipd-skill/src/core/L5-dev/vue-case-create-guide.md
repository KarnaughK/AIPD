...
- 需要在开发前确认组件嵌套、数据注入、事件流和 step 粒度。

不适用于后端、综合项目的跨端整体设计、L2/L3/L4 目标型研究。

## 前置设计

纯前端实现型 case 默认先做前端架构设计，不直接拆实现 step。

前置设计的核心不是把 UI 细节写满，而是先和用户对齐“文件黑箱如何嵌套”和“数据 / 事件如何单向流动”：

1. 先画 Mermaid 组件架构草图。除非目标明显是单文件微调，否则在创建实现 step 前，先用 `.mmd` 草图描述 `index.vue`、页面根组件、复杂弹框根组件或模块根组件如何挂载子组件。
...
纯前端实现型 case 的 step 拆分默认以文件为最小稳定单元：

- 一个要新建或重点改造的 `.vue` / `.js` / `.ts` 文件，默认对应一个 step；该 step 负责实现、校验、打磨和自检这个文件。
- 复制现有组件再改造成新组件，也按新文件单独成 step；不要因为来源是复制，就把它并入其他文件。
- 一个 step 可以读取多个上下文文件，但默认不同时创建多个业务文件。只有强耦合到无法独立验证的极小配套文件，才允许在同一 step 中说明原因后合并处理。
- 根组件和 `useXxx.js` / `useXxx.ts` 通常先于末端组件。推荐顺序是：架构图落位 -> `index.vue` / 根组件骨架 -> `useXxx` / provider / controller -> 直接子组件 -> 末端组件 -> 提交 / 切换 / 收尾校验。
- step 名称应直接指向目标文件，例如 `实现 DialogRoot.vue`、`细化 useCreateDialog.ts`、`实现 BlockForm.vue`，避免用“完善页面第一阶段”这类看不出文件边界的名称。

这里的“一个文件一个 step”是默认粒度，不是教条。若用户明确要求快速小改，或同一个文件内的局部改动无需 case-run 拆解，可以保持更轻；若一个文件过大且包含多个独立业务黑箱，也可以先在 case-create 中继续讨论是否需要拆文件。
