...
- 大调研、长日志分析、批量验证或子 Agent 派发前：写清当前问题、边界、预期输出、停止条件和返回位置。
...
进入 Execute phase 后，主 Agent 负责用户沟通、状态判断、派发、审查和写回；Main 或 Child 围绕 `03-execute/work-packages/` 下的 work package 运行。文件修改、构建、测试、批量验证、跨文件 diff、长调研和长日志分析按上下文隔离收益、并发收益和主线耦合度选择执行者，结果统一写回 case。
