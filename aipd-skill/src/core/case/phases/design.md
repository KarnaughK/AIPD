...
原有“复杂度爆点、最小必要解耦、并列扩展”仍然成立，但它不再是 Design 的唯一内容。它发生在需求、规则、后端和前端设计足够清楚之后，用来决定怎样让执行 Agent 低上下文推进，而不是纵向堆版本。
...
当主要交付物是产品文档、PRD、原型、用户流程、信息架构或用户可见页面，或者用户已经表达“太多、太复杂、重点不清、需要简化”时，读取匹配的产品设计经验，并在 `frontend-design.md` 或 `design.md` 写一份短的 Attention Contract：

- 用户此刻带着什么问题进入。
- 当前页面 / 段落唯一的主问题或主动作。
- 第一眼看见什么 -> 随后理解什么 -> 自然采取什么动作。
- 信息分别属于 `now / next / on-demand / remove` 哪一层。
- 这条判断应传播到当前页面、同类页面还是完整用户路径。
- 在不改变 Case Contract 的前提下，Agent 可以自主删除、合并、隐藏、延后或重排什么。
- Verify 观察到哪些现象时必须回到 Design。
- 什么证据代表已经收敛，继续迭代只会产生 churn。

这不是所有 Case 的必填流程；未涉及用户注意力的普通代码任务跳过。命中时也不预设必须修订几轮。当前设计在 Case Contract 内是可推翻的暂定解，不能因为已经实现就免于删除或重排。

信息优先级不能只按频率判断，还要看当前相关性、决策必要性、后果 / 紧迫性和可逆性。低频但不可逆的信息可以前置；高频但可预测的信息可以压缩或渐进披露。
...
如果本 Case 会新增或调整模块 / 文件夹边界、shared / domain / utils 上移、跨上下文依赖、组合协议或 Work Package 的独立验收边界，在 brownfield / backend / frontend 必要事实清楚后、Context Boundary 固化前读取 `@references/ai-friendly-code-topology.md`。

命中后，Design 必须形成项目具体的 `Code Topology Contract`，至少写清：

- 拓扑敏感：是 / 否。
- 本次纵向业务上下文及其目录 / 文件 owner。
- 允许依赖的横向基座与横向共享能力。
- 跨上下文的显式输入输出、事件或协议。
- 禁止的边界穿透、隐式引用和主干回流。
- 是否允许新增 / 上移 shared；未明确时默认为不允许。
- 当前上下文的独立验收边界和稳定认知回写位置。

Design 负责做出拓扑决策。Execute 只执行 Work Package 中的短护栏；如果实现需要改变合同，应回到 Design，不在 Execute 临场重做拓扑设计。
...
新的 Design 思路应像向外平摊的二层小洋楼：先找到复杂度爆点，给爆点一个可组合的结构，再让后续模块在同一架构边界内各自扩展。
...
- 拓扑敏感的 Case 是否形成了 Code Topology Contract，且共享变化权限、组合边界和 Verify 审计入口清楚？
- work package 是否按可验收目标包切，而不是微步骤？
- Verify 是否知道要检查哪些设计护栏？
- 如果命中产品注意力契约，是否已经写明真实产物的 Reduction Scan 入口、回跳触发器、传播范围和自适应停止条件？
