...
```
❌ 错误：美甲 + 美睫（两个平行方向）
✅ 正确：美业（上提到父层级，美甲和美睫作为 Product 功能线）
```
...
具体功能放到 `_aipd/knowledge/product/` 各模块里。

### 4. Intent 的"三不"原则

- ❌ **不描述具体功能**：如"一键验证"是功能，应该说"快速验证"
- ❌ **不描述技术实现**：如"SQLite 存储"是实现，不应出现在 Intent
- ❌ **不描述操作流程**：如"先标记再验证"是流程，应该放到 `_aipd/knowledge/product/` 各模块
...
在生成 Intent 前，必须检查以下具体化信号：

**识别具体化信号**：
- 具体工具名称（Dify、GPT-4、Notion）
- 具体功能描述（版本管理、一键验证、自动部署）
- 数据结构（v1/v2/v3、JSON 格式）
- 团队规模（3-5 人、10 人以下）
- 部署方式（不做登录、使用 Docker）
...
**延迟具体化**：
- ❌ 不在 Intent 讨论中追问"与 Dify 如何集成"
- ❌ 不在 Intent 讨论中追问"案例的数据结构"
- ❌ 不在 Intent 讨论中追问"是否需要登录"
- ✅ 按任务需要把这些问题路由到 Product、Core 或 Engineering 知识域
...
参见 `@references/knowledge/intent/template.md`

只有用户明确确认的长期方向、目标和边界才能写入 Intent；未确认候选继续留在讨论或当前 Case。
