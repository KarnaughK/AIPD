...
不适用于后端、综合项目的跨端整体设计，以及 Research / Core / Product 知识域的目标型研究。
...
组件图画法读取 `@references/knowledge/engineering/vue-architecture-diagram-guide.md`，不要把 Mermaid 细节复制回 `aipd-case/SKILL.md`。
...
涉及 `useXxx.ts/js` 细节时，再读取 `@references/knowledge/engineering/vue-provider-guide.md`。
