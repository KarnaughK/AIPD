...
- **Case**：一次马上要推进并最终关闭的短周期目标容器，记录 Goal / Think / Design / Execute / Verify / Close 状态、上下文索引、设计边界、工作包和归档信息。
- **Work Package / Step**：Case Execute phase 内可派发给子 Agent 的目标包；不是微步骤，而是围绕 Design 边界的可执行、可恢复、可验收工作包。新建 work package 放在 `04-execute/work-packages/`。`Step` 只作为旧称识别；旧 `steps/` 结构不再兼容运行，需先迁移为 phase-first case。
- **SOP**：以 Agent 为运行时的可复用 AI 原生程序，记录项目动作如何按步骤重复执行。
- **OKR**：阶段目标，用来判断工作是否推进了项目方向。
- **Weave**：把讨论、work package 结果、case 归档、代码 diff、错误日志和外部资料中的稳定信息，编织回项目 `_adoc/`、局部 README 或 map；一次性过程留在 case / work package。
...
- 当前执行到哪个 case、哪个 phase、哪个 work package，以 `_adoc/case/index.md`、对应 `case.md`、phase 目录和 `04-execute/work-packages/` 文件为准。
- 如果目标 case 仍是旧结构（例如顶层 `doc/`、`steps/`，但缺少 phase 目录），不要读取旧 `steps/` 继续执行；先提示用户是否迁移为 phase-first case。
...
- 每个会改变项目状态的 work package 完成后，必须把可恢复状态写回 work package 执行记录、`04-execute/execute.md` 和 case 状态。
- 在 `aipd-case` Execute phase 内，`04-execute/work-packages/` 下的 work package 是默认的子 Agent 派发节点。Main Agent 默认只做入口读取、状态判断、派发、审查摘要、验收和状态写回；work package 的具体执行默认交给子 Agent。
