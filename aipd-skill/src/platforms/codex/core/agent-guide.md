...
Codex 版本使用 Codex 子 agent 技术承载 AIPD 的上下文分身和角色执行 Agent。
...
Codex 子 agent 在技术上仍叫子 agent，但 AIPD 语义中要区分两类用法：
...
AIPD 不再把 `fork_context: true` 作为所有 step 的默认规则。它的核心价值是从 Main Agent 当前认知状态 fork 出一个上下文分身，让分身继承用户意图、当前判断和上下文方向，承担读取、扫描、验证、diff、长日志和调研等过程成本；Main Agent 只接收压缩后的结果回流。
