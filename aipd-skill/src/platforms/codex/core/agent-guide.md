...
Codex goal 是平台运行时目标，不是 AIPD 的长期记忆，也不是子 Agent。它和 Case / Work Package 一起提高 Main 在长任务、上下文压缩或续跑时的连续性，因此“可能压缩”本身不构成派发理由。
...
- work package 仍按 `aipd-case` 的运行时判定选择 Main 或 Child；goal 不替代状态写回，也不强制子 Agent 派发。
...
4. 判断是否启用 goal：多 work package、长任务、可能跨轮次或上下文压缩时启用；单个短 work package 或只读状态检查可以不启用。
