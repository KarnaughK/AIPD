...
- Leader 负责方向澄清、探索、Case 拆分、同级执行层调度和总验收；每个执行层对应一个 Case。Codex 上直接开独立 Codex 任务；Cursor 上因对话内 Agent 不够强，才用已登录的 `cursor-agent`。同一 Case 的 phase 回跳留在该执行层，不要为了推完它再开平级 Case。
