...
Codex 子 agent 是 AIPD 项目的通用分身探索能力，不只属于 case-run。普通对话、轻量修改、调研、验证和正式 case 都使用同一套逻辑：Main Agent 判断某段过程不应留在主线时，就 fork 子 agent 进入局部未来分支；case-run 只是把这个分支节点固化为 step。
...
AIPD2 推荐 `fork_context: true`，因为 Codex 子 agent 的核心价值不是“拿一份任务单去当外包工人”，而是从 Main Agent 当前认知状态 fork 出一个同源分身。Sub Agent 继承相同的项目认知、用户意图、当前判断和上下文方向，进入一条局部未来分支，承担读取、扫描、验证、diff、长日志和调研等过程成本；Main Agent 只接收压缩后的结果回流。

这不是状态机式的派工规则。判断是否创建子 Agent 时，重点不是任务智力难度，也不是总 token 消耗，而是探索过程是否应该留在主线里。只要过程会吞噬 Main Agent 的上下文预算、任务连续性或判断稳定性，就优先 fork 子 Agent。

case / step 文件仍然重要，但它们不是为了替代同源继承，而是用于校准任务边界、记录长期事实源，并在上下文压缩、长任务续跑或跨会话恢复时提供锚点。

非 case 模式下也可以直接 fork 子 agent，只要 prompt 明确局部目标、边界和返回格式。任务一旦变成长任务、需要验收归档或需要跨会话恢复，再补 case / step。
...
5. 派发子 Agent，默认使用 `fork_context: true`；prompt 用于声明身份、局部目标、边界和返回格式，而不是重新灌入完整上下文。
6. 子 Agent 完成后，主 Agent 只读取结果回流：结论、依据、风险、建议、改动文件和验证结果。
7. 成功则更新 step 执行记录和 case 状态，确保压缩后可恢复，再继续派发下一步。
8. 失败则告知用户，询问重试、跳过或手动处理。

## Main Agent 执行边界

Main Agent 保护的是主线连续性。case-run 链路内，以下任务默认适合进入子 Agent 的局部未来分支：
