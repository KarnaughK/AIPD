...
Codex 版本使用 Codex 子 agent 技术创建 AIPD2 分身 Agent。
...
Codex 子 agent 在技术上仍叫子 agent，但在 AIPD 语义中应理解为分身 Agent：它是主 Agent 的克隆体，不是被派工的低上下文执行者。普通对话、轻量修改、调研、验证和正式 case 都使用同一套逻辑：Main Agent 判断某段过程不应留在主线时，就 fork 分身 Agent 进入局部探索分支；case-run 只是把这个分支节点固化为 step。
...
Codex 分身 Agent 有两种上下文方式：

- `fork_context: true`：继承主 Agent 当前上下文。AIPD2 默认使用这种方式。
- `fork_context: false`：不继承主 Agent 当前上下文，只在需要隔离敏感上下文、测试独立可恢复性，或平台能力限制时使用。

AIPD2 推荐 `fork_context: true`，因为 Codex 分身 Agent 的核心价值不是“拿一份任务单去当外包工人”，而是从 Main Agent 当前认知状态 fork 出一个克隆体。分身 Agent 继承相同的项目认知、用户意图、当前判断和上下文方向，进入一条局部探索分支，承担读取、扫描、验证、diff、长日志和调研等过程成本；Main Agent 只接收压缩后的结果回流。

这不是状态机式的派工规则。判断是否创建分身 Agent 时，重点不是任务智力难度，也不是总 token 消耗，而是探索过程是否应该留在主线里。只要过程会吞噬 Main Agent 的上下文预算、任务连续性或判断稳定性，就优先 fork 分身 Agent。

case / step 文件仍然重要，但它们不是为了替代同源继承，而是用于校准任务边界、记录长期事实源，并在上下文压缩、长任务续跑或跨会话恢复时提供锚点。
...
4. 根据 step 头部 `推荐 Agent` 字段选择分身 Agent 类型；没有声明时再兜底判断：调研用 `explorer`，Vue 架构 / Vue 实现优先用 `aipd_vue_architect`，其他开发任务用 `worker`。
5. 派发分身 Agent，默认使用 `fork_context: true`；prompt 用于声明“你是分身 Agent / 克隆体”、局部目标、边界和返回格式，而不是重新灌入完整上下文。
6. 分身 Agent 完成后，主 Agent 只读取结果回流：结论、依据、风险、建议、改动文件和验证结果。
7. 成功则更新 step 执行记录和 case 状态，确保压缩后可恢复，再继续派发下一步。
8. 失败则告知用户，询问重试、跳过或手动处理。

## Main Agent 执行边界

Main Agent 保护的是主线连续性。case-run 链路内，以下任务默认适合进入分身 Agent 的局部探索分支：
...
你继承了 Main Agent 当前上下文。看到这句话后，不要再继续分身；你就是被 fork 出来的克隆体，负责完成当前 step 并回流结果。
...
你继承了 Main Agent 当前上下文。看到这句话后，不要再继续分身；你就是被 fork 出来的克隆体，负责完成当前调研并回流结果。
...
- 默认 fork 主 Agent 上下文，使用分身 Agent 模型保护 Main Agent 主线。
- prompt 必须包含 step 文件绝对路径。
- prompt 必须包含 step 的推荐 Agent 或兜底选择结果。
- prompt 必须包含 case 目录或 case.md 路径，以及本 step 需要遵守的上下文边界。
- 分身 Agent 必须自己读取 step 文件和上下文文档，用它们校准任务和恢复锚点。
- 主 Agent 不重复执行分身 Agent 已承担的任务，只负责入口读取、状态判断、派发、审查摘要、验收、状态更新和下一步调度。
