...
3. 整理 Weave Candidate，交给主 Agent 决定是否运行 `aipd2-weave`
...
根据改动内容，判断哪些信息可能需要交给 `aipd2-weave` 回写：
...
- 一次性执行过程、临时决策、验收记录 → 保留 case / step，不建议回写长期 ADOC。
...
1. **不直接更新长期 ADOC 正文**：稳定知识交给 `aipd2-weave` 判断和回写
2. **保持候选简洁**：不写冗余的描述
...
**Q: 不确定某个文档是否需要更新怎么办？**
A: 保守原则，不确定就不写长期 ADOC。把它放进 Weave Candidate，让主 Agent 和 `aipd2-weave` 判断。
...
**Q: 文档更新后需要用户确认吗？**
A: 长期 ADOC 回写需要由 `aipd2-weave` 先给方案再确认。归档分身只更新 case 索引、移动 case，并整理候选。
